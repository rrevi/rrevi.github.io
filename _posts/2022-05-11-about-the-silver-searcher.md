...
First, we're going to be searching for the text _silver_ in a directory with the following number of files:
...
Then, using the `grep` tool (the original of them all), we search for the _silver_ text in all files in said directory (and all of its sub-directories, recursively):
...
From the output above, we care for the value of _cpu_ and _total_, in this case meaning that the `grep` command used up to 55% of the available CPU and that it took 3.28 seconds from the moment the user hit the _Enter_ key to the moment the command line tool returned its output.
...
From the output above, using the `ack` tool to search for the same text across the same number of files, we see that it only used up to **27%** of the CPU (only about half of what `grep` required!) and it took **943 milliseconds** to run (over **3x** as fast as `grep`!!)
...
If you were impressed with the performance gains from using `ack` versus when using `grep`, well take a look at the results from using the `ag` tool, it used **147%** of the CPU and it only took **311 milliseconds** to run, which is to say that by using up more than 100% of the CPU the tool used multiple cores! and it was **3x** as fast as the `ack` tool (which was already fast! and if you do the math, this is over **10x** as fast as the original text search tool `grep`!!).
